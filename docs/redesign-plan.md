...
| A7    | `refactor/catalogue-redesign-07-workspace`       | Landed. The record page gains Preview (the student course view rendered from `admin_snapshot_projection`; a readable rendering for structures), Edit (every section as typed forms; course requisite rules through the tree editor; structure requirement trees shown as recorded until B5), and History with Restore and Discard. Manual saves create `origin = manual` snapshots based on the edited one and close open changes on edited fields. `docs/catalogue-operations.md` is the operating guide. Browser walkthroughs cover directory, review and workspace.                                                                                                             |
...
### Decisions taken during A5 and A6

- An import run holds one kind. Queue a programme and its majors as two runs.
- Acknowledging a blocking flag requires a note; warnings do not.
- A rejected change is not remembered. The next run recomputes against the
  new baseline.
