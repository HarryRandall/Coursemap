...
| A4    | `refactor/catalogue-redesign-04-requirements`    | The shared `requirement_*` tables and the condition vocabulary. Update `requisite-conditions.ts`, the editor components, `published_requirement_graph`, structure requirement display and `planner.ts`. Drop both old rule sets.                                                                                                                                                                                                    |
| A5    | `refactor/catalogue-redesign-05-import-pipeline` | Shared import tables and the single `lib/catalogue-import/` pipeline with kind adapters, one consumer, one admin API, run and target lifecycle functions with per-target recovery, directory and import list interfaces. `test:catalogue-db` rewritten.                                                                                                                                                                             |
| A6    | `refactor/catalogue-redesign-06-review`          | `catalogue_import_changes`, baseline diff in `changes.ts`, `apply_catalogue_import_changes` and `publish_catalogue_snapshot` on the new gate. Review workspace shows changes and flags per target.                                                                                                                                                                                                                                  |
| A7    | `refactor/catalogue-redesign-07-admin-workspace` | Shared workspace, manual editing, history and preview components for every kind. Programme import walkthrough end to end with Playwright. Majors, minors and specialisations verified against the same components. New guides replace the removed ones.                                                                                                                                                                             |
...
A3 was split after measuring it: 47 database functions (about 207,000
characters of PL/pgSQL) and roughly 40,000 lines of pipeline, admin interface
and tests sit on the six core tables, and two thirds of that is rewritten by
A5 to A7 regardless. Adapting it to the new tables first and rewriting it
again later would double the work, so A3a deletes it and A3b rebuilds only the
student-facing surface. Administrators cannot import or edit catalogue content
